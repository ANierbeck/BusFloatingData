    ...
    cat &> /opt/smack/conf/init_cluster_spark_job.json << EOF
{
  ...
  "run": {
    ...
    "args": ["bin/spark-submit",
             ...
             "https://oss.sonatype.org/content/repositories/snapshots/de/nierbeck/floating/data/spark-digest_2.11/0.3.0-SNAPSHOT/spark-digest_2.11-0.3.0-SNAPSHOT-assembly.jar",
    ...
    cat &> /opt/smack/conf/ingest.json << EOF
{
  ...
  "container": {
    ...
    "volumes": [],
    "docker": {
      "image": "anierbeck/akka-ingest:0.3.0-SNAPSHOT",
      "network": "HOST",
      "privileged": false,
      "parameters": [],
      "forcePullImage": true
    ...
    cat &> /opt/smack/conf/dashboard.json << EOF
{
    ...
    "container": {
        "type": "DOCKER",
        "docker": {
            "image": "anierbeck/akka-server:0.3.0-SNAPSHOT",
            "network": "HOST",
            "forcePullImage": true
        }
    },
